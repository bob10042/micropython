...
int main(void)
{
    ...
    UART_Print("  - DAC1:   12-bit, Channel 1 (PA4)\r\n");
    UART_Print("  - TIM3:   PWM on CH1 (PB4) - 1kHz\r\n");
    UART_Print("  - RTC:    Real-Time Clock with backup\r\n");
    UART_Print("  - RNG:    Hardware Random Number Generator\r\n");
    UART_Print("  - CAN1:   500kbps (PD0/PD1)\r\n");
    UART_Print("============================================================\r\n");
    ...
    /* Run GPIO test - all pins HIGH then LOW */
    GPIO_TestAllPins();
    ...
    while (1)
    {
        ...
        if (HAL_GetTick() - lastTick >= 100)
        {
            ...
            if (++printCount >= 10)
            {
                ...
                sprintf(txBuffer, "DAC:%4lu ADC:%4lu PWM:%4u RNG:0x%08lX %s\r\n",
...
void GPIO_TestAllPins(void)
{
    ...
    UART_Print("\r\n--- GPIO Test: Setting All D0-D10 HIGH ---\r\n");
    
    /* Set all HIGH */
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9 | GPIO_PIN_14, GPIO_PIN_SET);      /* D0, D1 */
    HAL_GPIO_WritePin(GPIOF, GPIO_PIN_15 | GPIO_PIN_14 | GPIO_PIN_13 | GPIO_PIN_12, GPIO_PIN_SET);  /* D2,D4,D7,D8 */
    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13 | GPIO_PIN_11 | GPIO_PIN_9, GPIO_PIN_SET);  /* D3,D5,D6 */
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15 | GPIO_PIN_14, GPIO_PIN_SET);     /* D9, D10 */
    
    HAL_Delay(10); /* Small delay for GPIO to settle */
    
    /* Read back and verify HIGH */
    UART_Print("Verifying HIGH state:\r\n");
    
    if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_9) == GPIO_PIN_SET) { UART_Print("  D0 (PG9)  = H [OK]\r\n"); pass++; }
    else { UART_Print("  D0 (PG9)  = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_14) == GPIO_PIN_SET) { UART_Print("  D1 (PG14) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D1 (PG14) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_15) == GPIO_PIN_SET) { UART_Print("  D2 (PF15) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D2 (PF15) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_13) == GPIO_PIN_SET) { UART_Print("  D3 (PE13) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D3 (PE13) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_14) == GPIO_PIN_SET) { UART_Print("  D4 (PF14) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D4 (PF14) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_11) == GPIO_PIN_SET) { UART_Print("  D5 (PE11) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D5 (PE11) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_9) == GPIO_PIN_SET) { UART_Print("  D6 (PE9)  = H [OK]\r\n"); pass++; }
    else { UART_Print("  D6 (PE9)  = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_13) == GPIO_PIN_SET) { UART_Print("  D7 (PF13) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D7 (PF13) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_12) == GPIO_PIN_SET) { UART_Print("  D8 (PF12) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D8 (PF12) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_15) == GPIO_PIN_SET) { UART_Print("  D9 (PD15) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D9 (PD15) = L [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_14) == GPIO_PIN_SET) { UART_Print("  D10(PD14) = H [OK]\r\n"); pass++; }
    else { UART_Print("  D10(PD14) = L [FAIL]\r\n"); fail++; }
    
    UART_Print("Waiting 2 seconds...\r\n");
    HAL_Delay(2000);
    
    UART_Print("\r\n--- GPIO Test: Setting All D0-D10 LOW ---\r\n");
    
    /* Set all LOW */
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9 | GPIO_PIN_14, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOF, GPIO_PIN_15 | GPIO_PIN_14 | GPIO_PIN_13 | GPIO_PIN_12, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13 | GPIO_PIN_11 | GPIO_PIN_9, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15 | GPIO_PIN_14, GPIO_PIN_RESET);
    
    HAL_Delay(10); /* Small delay for GPIO to settle */
    
    /* Read back and verify LOW */
    UART_Print("Verifying LOW state:\r\n");
    
    if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_9) == GPIO_PIN_RESET) { UART_Print("  D0 (PG9)  = L [OK]\r\n"); pass++; }
    else { UART_Print("  D0 (PG9)  = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_14) == GPIO_PIN_RESET) { UART_Print("  D1 (PG14) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D1 (PG14) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_15) == GPIO_PIN_RESET) { UART_Print("  D2 (PF15) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D2 (PF15) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_13) == GPIO_PIN_RESET) { UART_Print("  D3 (PE13) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D3 (PE13) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_14) == GPIO_PIN_RESET) { UART_Print("  D4 (PF14) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D4 (PF14) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_11) == GPIO_PIN_RESET) { UART_Print("  D5 (PE11) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D5 (PE11) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_9) == GPIO_PIN_RESET) { UART_Print("  D6 (PE9)  = L [OK]\r\n"); pass++; }
    else { UART_Print("  D6 (PE9)  = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_13) == GPIO_PIN_RESET) { UART_Print("  D7 (PF13) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D7 (PF13) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_12) == GPIO_PIN_RESET) { UART_Print("  D8 (PF12) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D8 (PF12) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_15) == GPIO_PIN_RESET) { UART_Print("  D9 (PD15) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D9 (PD15) = H [FAIL]\r\n"); fail++; }
    
    if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_14) == GPIO_PIN_RESET) { UART_Print("  D10(PD14) = L [OK]\r\n"); pass++; }
    else { UART_Print("  D10(PD14) = H [FAIL]\r\n"); fail++; }
    
    sprintf(buf, "\r\n>>> GPIO Test: %d PASS, %d FAIL <<<\r\n\r\n", pass, fail);
    UART_Print(buf);
}
...
static void GPIO_Init(void)
{
    ...
    /* Enable GPIO clocks for D0-D10 pins */
    __HAL_RCC_GPIOE_CLK_ENABLE();
    __HAL_RCC_GPIOF_CLK_ENABLE();
    __HAL_RCC_GPIOG_CLK_ENABLE();
    
    /* Configure D0-D10 as outputs */
    /* D0=PG9, D1=PG14 */
    ...
    /* D2=PF15, D4=PF14, D7=PF13, D8=PF12 */
    GPIO_InitStruct.Pin = GPIO_PIN_15 | GPIO_PIN_14 | GPIO_PIN_13 | GPIO_PIN_12;
    HAL_GPIO_Init(GPIOF, &GPIO_InitStruct);
    
    /* D3=PE13, D5=PE11, D6=PE9 */
...
static void RTC_Init(void)
{
    ...
    /* Force reset backup domain to clear any previous RTC config */
    __HAL_RCC_BACKUPRESET_FORCE();
    __HAL_RCC_BACKUPRESET_RELEASE();

    /* Enable LSI oscillator with timeout */
    ...
    while (__HAL_RCC_GET_FLAG(RCC_FLAG_LSIRDY) == RESET)
    {
        ...
        {
            UART_Print("  [WARN] LSI timeout - RTC skipped\r\n");
            return;  /* Skip RTC if LSI fails */
        }
    }
    ...
    {
        UART_Print("  [WARN] RTC init failed - skipped\r\n");
        return;  /* Don't crash, just skip RTC */
    }
}
